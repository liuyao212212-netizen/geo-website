<template>
  <div ref="containerRef" class="splash-cursor-container">
...
const props = withDefaults(defineProps<SplashCursorProps>(), {
  ...
  BACK_COLOR: () => ({ r: 0.01, g: 0.0, b: 0.03 }),
  TRANSPARENT: true
});
...
let eventListeners: Array<{ target: EventTarget; event: string; handler: EventListener }> = [];
...
onMounted(() => {
  ...
  // Use container-scoped events instead of window
  addListener(container, 'mousedown', handleMouseDown as EventListener);
  addListener(container, 'mousemove', handleMouseMove as EventListener);
  addListener(container, 'touchstart', handleTouchStart as EventListener);
  addListener(container, 'touchmove', handleTouchMove as EventListener);
  addListener(container, 'touchend', handleTouchEnd as EventListener);

  // Start auto-splat for visual effect even without interaction
  function autoSplat() {
    const x = Math.random();
    const y = Math.random();
  ...
  // Initial splats for visual effect
  for (let i = 0; i < 5; i++) {
    autoSplat();
  }

  // Periodic auto-splat
  const autoSplatInterval = setInterval(autoSplat, 1500);
